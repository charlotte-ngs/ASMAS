...
BW <- round(10*PWG/2 + rnorm(nNrCalf), digits = 1)
dfGainData <- data.frame(Calf, BW, WWG=2*WWG/10, PWG=2*PWG/10)
#dfGainData
#summary(dfGainData)
```
...
```{r OdgModelComponents}
rcoursetools::insertOdgAsPdf(psOdgFileStem = "TableGainData", pnPaperWidthScale = 0.8)
```
...
## Keine Extrapolation
```{r PillKink}
rcoursetools::insertOdgAsPdf(psOdgFileStem = "PillKink", pnPaperWidthScale = 0.8)
```
